...
import { Data, getSingleDate, updateRecord } from "@/actions/file";
...
export default function UpdateRecord({data}) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    try {
      const response = await updateRecord({id:data.id, province, city, population });
    ...
    <main>
      <div className="flex flex-col gap-5 justify-center items-center h-screen">
        ...
        <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
          ...
          <select defaultValue={data.province} id="province" onChange={handleProvinceChange} className="border-black border-2 px-2 py-1 rounded-md">
          ...
          <select defaultValue={data.city} id="city" onChange={handleCityChange} className="border-black border-2 px-2 py-1 rounded-md">
          ...
          />
          <p>{message}</p>
          <button type="submit" className="p-2 bg-red-600 rounded-md">Update</button>
        </form>
      </div>
    </main>
  );
